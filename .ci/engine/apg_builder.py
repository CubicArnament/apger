...
class APGPackageBuilder:
    ...
    def _combine_metadata(
        self, json_metadata: dict[str, Any], yaml_metadata: dict[str, Any]
    ) -> dict[str, Any]:
        """
            ...
            for key, value in yaml_metadata["package"].items():
                ...
                match key in combined["package"]:
                    case True:
                        ...
                        match isinstance(combined["package"][key], list) and isinstance(
                            value, list
                        ):
                            case True:
                                combined["package"][key] = list(
                                    set(combined["package"][key] + value)
                                )  # Убираем дубликаты
                            case False:
    ...
    def _load_filesystem_config(self, yaml_path: str) -> dict[str, Any]:
        ...
        if yaml_file.exists():
            ...
            self.logger.warning(
                "Файл конфигурации файловой системы не найден: %s", yaml_path
            )
            return {}

    def _apply_filesystem_config(
        self, data_dir: Path, filesystem_config: dict[str, Any]
    ) -> None:
        """
    ...
    def _generate_metadata(self, package_info: dict[str, Any]) -> dict[str, Any]:
        ...
        install_size = self._calculate_installed_size(
            Path(package_info.get("install_dir", ""))
        )
